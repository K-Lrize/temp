...
func TestBuildManifestPullsFromVariantAndFingerprints(t *testing.T) {
	v := metaVariant(true)
	fp := plan.Fingerprints{LineTree: "lt", Line: "L", Feed: "F", Variant: "V"}
	m := buildManifest(v, fp, "r-123", "b-456", "6.12.94-1-abc", "http://ci/run/1", epoch)

	// 参数直传的
	if m.ReleaseID != "r-123" || m.BuildID != "b-456" || m.Vermagic != "6.12.94-1-abc" || m.CIRunURL != "http://ci/run/1" {
		t.Errorf("参数没原样进小票: %+v", m)
	}
	// 从 variant 自动取的
	...
	// 三层指纹映射
	if m.Fingerprints.Line != "L" || m.Fingerprints.Feed != "F" || m.Fingerprints.Variant != "V" {
		t.Errorf("三层指纹没映射对: %+v", m.Fingerprints)
	}
	// 时间戳：UTC、RFC3339
...
func TestBuildManifestOfficialHasNoCommitNoBuildID(t *testing.T) {
	// official 线没有自建工具链，没有 build_id，也没有 source commit。
	m := buildManifest(metaVariant(false), plan.Fingerprints{}, "r-1", "", "vm", "", epoch)
	if m.UpstreamCommit != "" {
...
func TestBuildBuildKeepsTreeAndCommitSeparate(t *testing.T) {
	...
	b := buildBuild(v, fp, "b-1", "vm", "6.12.94", "sdksha", "ibsha", 217, "http://ci", epoch)
	...
	if b.KmodCount != 217 || b.SDKSHA256 != "sdksha" || b.IBSHA256 != "ibsha" {
		t.Errorf("构建事实没原样进小票: %+v", b)
	}
}

func TestMetaLatestEmitsJSON(t *testing.T) {
	stdout, _, err := runCLI(t, atRepo(t, "meta", "latest", "--release-id", "r-abc")...)
	if err != nil {
		t.Fatal(err)
	}
	var l struct {
